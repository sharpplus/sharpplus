...
public class SharpMultiProver {
	...
	public SharpMultiProver() throws NoSuchAlgorithmException {
		pc = PedersenCommitment.getDefault();
		generator = new Random();
		transcript = new Transcript();
	}
	
	public SharpMultiProof generateMultiProof(BigInteger[] x, BigInteger[] y1, BigInteger[] y2, BigInteger r_x) {
		int N = x.length;
...
//		BigInteger gamma_star = SharpUtils.drawNumberFromSeed(new byte[] {});
		...
		for(int i=0;i<N;i++) {
			alpha_1[i] = gamma_star.pow(i).multiply(y1[i].multiply(tilde_y2[i]).add(y2[i].multiply(tilde_y1[i]))).multiply(BigInteger.valueOf(-1));
			alpha_0[i] = gamma_star.pow(i).multiply(tilde_x[i].subtract(tilde_y1[i].multiply(tilde_y2[i])));
		}
