...
public class BpVerifier extends ConstraintSystem {

    private final Transcript transcript;

    private final List<LinearCombination> constraints = new ArrayList<>();

    private final List<Consumer<BpVerifier>> deferredConstraints = new ArrayList<>();

    private final List<ECPoint> values = new ArrayList<>();

    private int numVars = 0;

    public BpVerifier(Transcript transcript) {
    ...
    private void randomizedConstraints() {
        if (deferredConstraints.isEmpty()) {
            ...
            for (Consumer<BpVerifier> fn : deferredConstraints) {
                fn.accept(this);
            }
        }
    }
    ...
    public boolean verify(BpProof proof, PedersenCommitment pedersenCommitment, BulletProofGenerators generators) {
